	<?php
	global $UserSidebarIDs,$selectedTemplateID, $header_background_image, $header_text,$boxNumber;  
			...
			if ($lastBox || $boxNumber == count($selectedTemplate_contents) ) {
				...
				$reset = false;
					...
					if($template->content_type=="portfolio_box"){ 				
						...
						$item_width = $template->item_width; //item width
						$layout     = $template->layout; 
						$selectedPortfolioCatetegories  = isset($template->categories) ? $template->categories : "";
						$filterable  = @$template->filterable ? @$template->filterable : "";
								...
								$args=array( 
									...
									'tax_query' => array(
										array(
											...
											'field'    =>	'id',
											'terms'    =>	 isset($template->categories) ? $template->categories : '',
											'operator' => 	 isset($template->categories) ? "IN" : "OR"
										)
									),																		
								);
					...
					if($template->content_type=="code_box"){ 			 
						...
						$inlineCSS1    = $transparent ? "background:none;" : "";
						$inlineCSS2    = $no_padding ? "padding:0;" : ""; 
						$group_id      =  $template->group_id;  

						if($layout!="one"){
							if($transparent)
								echo '<div id="code_box_'.$template->group_id.'" style="'.$inlineCSS1.' '.$inlineCSS2.'" class="template_builder box '.$layout.' '.$firstBox.'  '.$lastBox.'">';
							else
								echo '<div id="code_box_'.$template->group_id.'" style="'.$inlineCSS2.'" class="template_builder box-shadow box '.$layout.' '.$firstBox.'  '.$lastBox.'">';
						}else{
							if($transparent)
								echo '<div id="code_box_'.$template->group_id.'" style="'.$inlineCSS1.' '.$inlineCSS2.'" class="template_builder  '.$layout.' '.$firstBox.'  '.$lastBox.'">';
							else
								echo '<div id="code_box_'.$template->group_id.'" style="'.$inlineCSS2.'" class="template_builder box-shadow box '.$layout.' '.$firstBox.'  '.$lastBox.'">';							
						}
