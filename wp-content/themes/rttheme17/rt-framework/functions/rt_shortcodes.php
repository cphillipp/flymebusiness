...
function rt_shortcode_products( $atts, $content = null ) {
    global $post;
    //[product_showcase categories="product-category-1,product-category-2" columns="2" limit="2" desc="false"]
    ...
    if($ids){ //ids provided
    $queryProducts   = new WP_Query(array(  'post_type'=> 'products', 'post_status'=> 'publish' ,  'post__in'  => $ids, 'showposts' => $limit ));
    }else{ //product slugs provided    
    $queryProducts   = new WP_Query(array(  'post_type'=> 'products', 'post_status'=> 'publish' ,   'showposts' => $limit,				 
								    'tax_query' => array( 
										array(
