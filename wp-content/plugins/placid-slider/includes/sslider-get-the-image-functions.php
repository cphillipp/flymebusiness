<?php
/**
 * Almost all of the below code belongs to 'Get the Image' WordPress plugin by Justin Tadlock. This is the perfect code for extracting post images, I have just changed the function * names specific to Placid Slider, as someone would like to use the 'Get the Image' WordPress plugin separately. 
 ...
 * is found, call placid_sslider_display_the_image() to format it.
 *
 * @since 0.1
 ...
 */
function placid_sslider_get_the_image( $args = array() ) {
	global $post;
	$post_id = $post->ID;
	...
	$args = apply_filters( 'placid_sslider_get_the_image_args', $args );
 ...
 * @return array|bool
 */
function placid_sslider_image_by_custom_field( $args = array() ) {
 ...
 * other filters are later added in the placid_sslider_display_the_image() function.
 *
 * @since 0.4
 * @param array $args
 * @return array|bool
 */
function placid_sslider_image_by_the_post_thumbnail( $args = array() ) {
 ...
 * @return array|bool
 */
function placid_sslider_image_by_attachment( $args = array() ) {
...
/**
 * Scans the post for images within the content.  Not called by default with placid_sslider_get_the_image().
 ...
 * @return array|bool
 */
function placid_sslider_image_by_scan( $args = array() ) {
 ...
 * given in an array.  Not used with placid_sslider_get_the_image() by default.
 *
 * @since 0.3
 * @param array $args
 * @return array
 */
function placid_sslider_image_by_default( $args = array() ) {
 ...
 */
function placid_sslider_display_the_image( $args = array(), $image = false ) {
	...
	if($timthumb=='1'){
		...
		if (isset($blog_id) && $blog_id > 0) {
			$imageParts = explode('/files/', $image['url']);
			if (isset($imageParts[1])) {
				$image['url'] = 'http://' . $current_site->domain . $current_site->path . 'wp-content/blogs.dir/' . $blog_id . '/files/' . $imageParts[1];
			}
		}
	}
	
	//Prettyphoto Integration	
	if($imglink=='1' and $permalink!='') $permalink=$image['url'];
	elseif($imglink=='') $permalink=$permalink;
	else {
		if($permalink!='')$permalink=$imglink;
	}
	
	/* Add the image attributes to the <img /> element. */
	if($timthumb=='1'){
	  $html = '<img src="'.placid_slider_plugin_url( 'includes/timthumb.php' ) .'?src='. $image['url'] . '&amp;h='.$h.'&amp;w='.$w.'" alt="' . esc_attr( strip_tags( $image_alt ) ) . '" class="' . esc_attr( $class ) . '"' . $width . $height . $style .' />';	  
    }
	else{
...
/**
 * Get the image with a link to the post.  Use placid_sslider_get_the_image() instead.
 *
 * @since 0.1
 * @deprecated 0.3
 */
function placid_sslider_get_the_image_link( $deprecated = '', $deprecated_2 = '', $deprecated_3 = '' ) {
	placid_sslider_get_the_image();
}
